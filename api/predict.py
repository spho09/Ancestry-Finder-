...
def load_model():
    """Loads the reference model. The file stores only numeric (float32/
    uint8) arrays -- no string-dtype arrays -- to avoid numpy version
    compatibility issues between build and deploy environments. Text
    metadata (SNP IDs, alleles, population labels) is packed as raw JSON
    bytes in the 'meta_json' array and decoded here."""
    global _model
    if _model is None:
        with np.load(MODEL_PATH, allow_pickle=False) as data:
            meta = json.loads(bytes(data["meta_json"]).decode("utf-8"))
            _model = {
                "allele_freq": data["allele_freq"],
                "pca_mean": data["pca_mean"],
                "pca_components": data["pca_components"],
                "ref_pcs": data["ref_pcs"],
                "snp_ids": np.array(meta["snp_ids"]),
                "ref_allele": np.array(meta["ref_allele"]),
                "alt_allele": np.array(meta["alt_allele"]),
                "ref_super_pop": np.array(meta["ref_super_pop"]),
                "ref_pop": np.array(meta["ref_pop"]),
            }
    return _model
